...
"""
This file is an altered version of https://github.com/pybox2d/pybox2d/blob/master/library/Box2D/examples/backends/pygame_framework.py
"""
...
from external_lib.box2d_framework import (FrameworkBase, Keys)
from external_lib.box2d_settings import fwSettings
from Box2D import (b2DrawExtended, b2Vec2)
from tkinter import Tk, messagebox

try:
    from external_lib.pygame_gui import (fwGUI, gui)
    GUIEnabled = True
...
class PygameFramework(FrameworkBase):
    ...
    def __init__(self):
        ...
        pygame.init()
        caption = self.name + " - Python Box2D"
        ...
        if GUIEnabled:
            ...
            container.widgets[-1].disabled = True
    ...
    def checkEvents(self):
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == Keys.K_ESCAPE):
                # macOS has issues with Tkinter, skip popup GUI
                if sys.platform == "darwin":
                    return False
                else:
                    Tk().wm_withdraw()  # hide the main window
                    result = messagebox.askyesno(title="Confirm Exiting Simulator",
